...
class KommersantParser:
    ...
    async def get_entites(self, count = 40):
        try:
            links = await self._get_ajax_news_links(count)
    ...
    async def _get_ajax_news_links(self, count, delay = 1):
    ...
    async def _get_root_page_links(self):
        root_page = await http_client.get(KommersantParser.NEWS_ROOT_URL)
    ...
    async def _get_ajax_links_after(self, id):
        response = await http_client.get(self._construct_ajax_req_url(id))
